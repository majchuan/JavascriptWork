/*
ittle Bobby loves chocolate. He frequently goes to his favorite  store, Penny Auntie, to buy them. 
They are having a promotion at Penny Auntie. If Bobby saves enough wrappers, he can turn them in for a free chocolate.

Example
He has  to spend, bars cost , and he can turn in  wrappers to receive another bar. Initially, 
he buys  bars and has  wrappers after eating them. He turns in  of them, leaving him with , 
for  more bars. After eating those two, he has  wrappers, turns in  leaving him with  wrapper and his new bar. 
...
He spends  on  chocolates at  apiece. He then exchanges  of the  wrappers for  additional piece. 
Next, he uses his third leftover chocolate wrapper from his initial purchase with the wrapper from his trade-in to do a second trade-in 
for  more piece. At this point he has  wrapper left, which is not enough to perform another trade-in. He eats  chocolates.
*/
'use strict';
/*
...
function chocolateFeast(n, c, m) {
    ...
    let reminderWrappers = 0 ;
    let totalEaternNumber = wrappers ;
    while(wrappers >= m ){
        reminderWrappers = wrappers % m ;
        wrappers = Math.floor(wrappers / m );
        totalEaternNumber = totalEaternNumber + wrappers ;
        wrappers = wrappers + reminderWrappers ; 
    }
    return totalEaternNumber;
}
